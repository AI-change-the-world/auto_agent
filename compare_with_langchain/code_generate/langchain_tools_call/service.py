...
class ProjectService(BaseService):
    ...
    async def update_project(self, id: int, request: UpdateProjectRequest) -> ProjectResponse:
        ...
        return ProjectResponse(
            ...
            description=request.description,
            status=ProjectStatusEnum(request.status) if request.status else ProjectStatusEnum.ACTIVE,
            created_at=now,
            updated_at=now
        )
...
class ProjectMemberService(BaseService):
    ...
    async def add_project_member(self, project_id: int, request: AddProjectMemberRequest) -> ProjectMemberResponse:
        # Mock implementation
        if project_id <= 0:
            raise NotFoundException(f"Project with id {project_id} not found")
...
class TaskService(BaseService):
    ...
    async def create_task(self, request: CreateTaskRequest) -> TaskResponse:
        ...
        return TaskResponse(
            ...
            parent_task_id=str(UUID(int=request.parent_task_id)) if request.parent_task_id else None,
            title=request.title,
            description=request.description,
            priority=priority,
            due_date=request.due_date,
            status=status,
            assignee_id=str(UUID(int=request.assignee_id)) if request.assignee_id else None,
            created_at=now,
            updated_at=now
        )
    ...
    async def update_task(self, id: int, request: UpdateTaskRequest) -> TaskResponse:
        ...
        return TaskResponse(
            id=task_id,
            project_id=str(UUID(int=request.project_id)) if request.project_id else str(UUID(int=1)),
            parent_task_id=str(UUID(int=request.parent_task_id)) if request.parent_task_id else None,
...
class TagService(BaseService):
    ...
    async def add_task_tag(self, task_id: int, request: AddTaskTagRequest) -> TaskTagResponse:
        ...
        return TaskTagResponse(
            ...
            tag_id=str(UUID(int=request.tag_id))
        )
