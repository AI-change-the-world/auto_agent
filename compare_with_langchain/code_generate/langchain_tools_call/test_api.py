...
async def test_update_project_success(client: AsyncClient, auth_headers: Dict[str, str], sample_project_data: Dict[str, Any]):
    ...
    update_data = {"name": "Updated Project", "status": ProjectStatusEnum.COMPLETED.value}
...
async def test_remove_project_member_success(client: AsyncClient, auth_headers: Dict[str, str], sample_project_data: Dict[str, Any]):
    ...
    response = await client.delete(f"/projects/{project_id}/members/1", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert data == {}
...
@pytest.mark.asyncio
async def test_get_tasks_with_filters(client: AsyncClient, auth_headers: Dict[str, str]):
    """测试获取任务列表带过滤参数"""
    params = {"project_id": 1, "status": TaskStatusEnum.TODO.value, "page": 1, "size": 10}
    response = await client.get("/tasks", params=params, headers=auth_headers)
    assert response.status_code == 200


@pytest.mark.asyncio
async def test_create_task_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any]):
    """测试创建任务成功"""
...
@pytest.mark.asyncio
async def test_get_task_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any]):
    """测试获取单个任务成功"""
    create_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_update_task_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any]):
    """测试更新任务成功"""
    create_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = create_response.json()["id"]
...
@pytest.mark.asyncio
async def test_delete_task_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any]):
    """测试删除任务成功"""
    create_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = create_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_subtasks_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any]):
    """测试获取子任务列表成功"""
    # 创建父任务
    parent_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_create_tag_success(client: AsyncClient, auth_headers: Dict[str, str], sample_tag_data: Dict[str, Any]):
    """测试创建标签成功"""
...
@pytest.mark.asyncio
async def test_get_tag_success(client: AsyncClient, auth_headers: Dict[str, str], sample_tag_data: Dict[str, Any]):
    """测试获取单个标签成功"""
    create_response = await client.post("/tags", json=sample_tag_data, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_update_tag_success(client: AsyncClient, auth_headers: Dict[str, str], sample_tag_data: Dict[str, Any]):
    """测试更新标签成功"""
    create_response = await client.post("/tags", json=sample_tag_data, headers=auth_headers)
    tag_id = create_response.json()["id"]
...
@pytest.mark.asyncio
async def test_delete_tag_success(client: AsyncClient, auth_headers: Dict[str, str], sample_tag_data: Dict[str, Any]):
    """测试删除标签成功"""
    create_response = await client.post("/tags", json=sample_tag_data, headers=auth_headers)
    tag_id = create_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_task_tags_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_tag_data: Dict[str, Any]):
    """测试获取任务标签列表成功"""
    # 创建任务和标签
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
    
    tag_response = await client.post("/tags", json=sample_tag_data, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_add_task_tag_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_tag_data: Dict[str, Any]):
    """测试为任务添加标签成功"""
    # 创建任务和标签
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_remove_task_tag_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_tag_data: Dict[str, Any]):
    """测试从任务移除标签成功"""
    # 创建任务和标签，并关联
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
    ...
    await client.post(f"/tasks/{task_id}/tags", json={"tag_id": tag_id}, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_get_task_comments_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any]):
    """测试获取任务评论列表成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_create_comment_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_comment_data: Dict[str, Any]):
    """测试创建评论成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_comment_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_comment_data: Dict[str, Any]):
    """测试获取单条评论成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
    
    comment_response = await client.post(f"/tasks/{task_id}/comments", json=sample_comment_data, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_update_comment_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_comment_data: Dict[str, Any]):
    """测试更新评论成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_delete_comment_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_comment_data: Dict[str, Any]):
    """测试删除评论成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_task_attachments_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any]):
    """测试获取任务附件列表成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_create_attachment_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_attachment_data: Dict[str, Any]):
    """测试创建附件成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_attachment_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_attachment_data: Dict[str, Any]):
    """测试获取单个附件成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
    
    attachment_response = await client.post(f"/tasks/{task_id}/attachments", json=sample_attachment_data, headers=auth_headers)
...
@pytest.mark.asyncio
async def test_delete_attachment_success(client: AsyncClient, auth_headers: Dict[str, str], sample_task_data: Dict[str, Any], sample_attachment_data: Dict[str, Any]):
    """测试删除附件成功"""
    task_response = await client.post("/tasks", json=sample_task_data, headers=auth_headers)
    task_id = task_response.json()["id"]
...
@pytest.mark.asyncio
async def test_get_projects_boundary_pagination(client: AsyncClient, auth_headers: Dict[str, str]):
    """测试项目列表分页边界条件"""
    # 测试 page=0
    response = await client.get("/projects", params={"page": 0, "size": 10}, headers=auth_headers)
    assert response.status_code in [200, 422]  # 可能返回200（如果后端处理）或422（如果验证失败）
    
    # 测试 size 超出范围
    response = await client.get("/projects", params={"page": 1, "size": 1000}, headers=auth_headers)
    # 根据实际实现，可能返回200或422


@pytest.mark.asyncio
async def test_create_project_boundary_name_length(client: AsyncClient, auth_headers: Dict[str, str]):
    """测试项目名称长度边界条件"""
    # 测试超长名称
    long_name = "A" * 256
    response = await client.post("/projects", json={"name": long_name, "status": ProjectStatusEnum.ACTIVE.value}, headers=auth_headers)
    assert response.status_code == 422
