...
async def update_single_project(
    id: int = Path(..., gt=0), project_data: UpdateProjectRequest = None
):
...
async def add_member_to_project(
    project_id: int = Path(..., gt=0), member_data: AddProjectMemberRequest = None
):
...
async def remove_member_from_project(
    project_id: int = Path(..., gt=0), user_id: int = Path(..., gt=0)
):
...
async def list_users(page: int = Query(1, ge=1), size: int = Query(10, ge=1, le=100)):
...
async def update_single_task(
    id: int = Path(..., gt=0), task_data: UpdateTaskRequest = None
):
...
async def list_tags(page: int = Query(1, ge=1), size: int = Query(10, ge=1, le=100)):
...
async def update_single_tag(
    id: int = Path(..., gt=0), tag_data: UpdateTagRequest = None
):
...
async def add_tag_to_task(
    task_id: int = Path(..., gt=0), tag_data: AddTaskTagRequest = None
):
...
async def remove_tag_from_task(
    task_id: int = Path(..., gt=0), tag_id: int = Path(..., gt=0)
):
    success = await remove_task_tag(task_id, tag_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Tag not associated with task"
        )
    return EmptyResponse()
...
async def create_task_comment(
    task_id: int = Path(..., gt=0), comment_data: CreateCommentRequest = None
):
...
async def update_single_comment(
    id: int = Path(..., gt=0), comment_data: UpdateCommentRequest = None
):
...
async def create_task_attachment(
    task_id: int = Path(..., gt=0), attachment_data: CreateAttachmentRequest = None
):
