...
from auto_agent import OpenAIClient, func_tool, get_global_registry
        ...
        "违规收受礼品": [
            {
                "id": "GC2022020",
                "title": "某科长收受礼金案",
                "key_points": ["金额认定", "退还情节"],
            },
        ],
    }

    cases = cases_db.get(
        case_type, [{"id": "GC0000", "title": "通用案例", "key_points": ["程序规范"]}]
    )
...
async def search_research_papers(case_type: str) -> dict:
    ...
    papers = papers_db.get(
        case_type, [{"title": "纪检监察规范化研究", "insights": ["程序规范"]}]
    )
...
class ReportGenerator:
    ...
    def generate_html(
        ...
        memo = (
            state.get("generate_memo", {}).get("memo_content", "").replace("\n", "<br>")
        )
...
async def main():
    ...
    try:
        ...
        plan = ExecutionPlan(
            ...
            subtasks=[
                ...
                PlanStep(
                    id="2", tool="search_guidance_cases", description="查询指导性案例"
                ),
                PlanStep(
                    id="3", tool="search_research_papers", description="查询论文研究"
                ),
    ...
    print(
        f"总步骤: {len(results)}, 成功: {success_count}, 失败: {len(results) - success_count}"
    )
    print("📄 HTML 报告: llm_agent_report.html")
    print("📄 Markdown 报告: llm_agent_report.md")
