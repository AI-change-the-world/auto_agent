...
class LLMCallEvent(TraceEvent):
    ...
    def to_dict(self, truncate: bool = True) -> Dict[str, Any]:
        ...
        if truncate:
            prompt_display = (
                self.prompt[:500] + "..." if len(self.prompt) > 500 else self.prompt
            )
            response_display = (
                self.response[:500] + "..."
...
class ToolCallEvent(TraceEvent):
    ...
    def to_dict(self, truncate: bool = True) -> Dict[str, Any]:
        ...
        if truncate:
            args_display = (
                self.arguments if len(args_str) < 1000 else f"[{len(args_str)} chars]"
            )
            result_display = (
        ...
        base.update(
            {
                ...
                "binding_details": self.binding_details
                if not truncate
                else self.binding_details[:5],
            }
        )
        return base
...
class MemoryEvent(TraceEvent):
    ...
    def to_dict(self, truncate: bool = True) -> Dict[str, Any]:
        ...
        if truncate:
            ...
            content_display = (
                self.content_preview[:300] + "..."
                if len(self.content_preview) > 300
...
class TraceContext:
    ...
    start_time: float = field(default_factory=time.time)
    end_time: Optional[float] = None
    root_span: TraceSpan = field(
        default_factory=lambda: TraceSpan(name="root", span_type="root")
    )
    ...
    def _find_span_recursive(
        self, span: TraceSpan, span_id: str
    ) -> Optional[TraceSpan]:
        """递归查找跨度"""
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_ops": {
                ...
                "writes": sum(
                    1 for e in memory_events if e.action == MemoryAction.WRITE
                ),
                "searches": sum(
                    1 for e in memory_events if e.action == MemoryAction.SEARCH
                ),
            },
    ...
    def to_dict(self, truncate: bool = True) -> Dict[str, Any]:
        ...
        if truncate:
            query_display = (
                self.query[:500] + "..." if len(self.query) > 500 else self.query
            )
        else:
            query_display = self.query
