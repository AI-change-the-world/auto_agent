...
class RetryController:
    ...
    def _build_recovery_detail_content(self, record: ErrorRecoveryRecord) -> str:
        ...
        lines = [
            ...
            "```json",
            json.dumps(
                record.original_params, ensure_ascii=False, indent=2, default=str
            ),
            "```",
            "",
            "## 修正后参数",
...
请只返回 JSON，不要包含其他内容。"""
    ...
    def _fallback_error_analysis(self, exception: Exception) -> ErrorAnalysis:
        ...
        if any(
            kw in exception_type or kw in exception_message for kw in network_keywords
        ):
            return ErrorAnalysis(
        ...
        if any(
            kw in exception_type or kw in exception_message for kw in timeout_keywords
        ):
            return ErrorAnalysis(
        ...
        if any(
            kw in exception_type or kw in exception_message for kw in param_keywords
        ):
            return ErrorAnalysis(
        ...
        if any(
            kw in exception_type or kw in exception_message
        ...
        if any(
            kw in exception_type or kw in exception_message for kw in resource_keywords
        ):
            return ErrorAnalysis(
        ...
        if any(
            kw in exception_type or kw in exception_message for kw in logic_keywords
        ):
            return ErrorAnalysis(
